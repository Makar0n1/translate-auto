...
const promptTemplates = {
  ...
  metaDescription: (title, lang) => `You are an SEO specialist for a streaming platform. Generate a concise, SEO-optimized meta description (100-160 characters) in ${lang} based solely on the movie or series title "${title}". Include relevant keywords, a compelling hook, and a call-to-action. Ensure it is natural and engaging for the ${lang}-language audience. Do not include quotes, brackets, or formatting. Return only the meta description.`
};
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      const response = await openai.chat.completions.create({
        ...
        max_tokens: type === 'description' || type === 'generateDescription' ? 1500 : type === 'metaDescription' ? 100 : 500
      });
